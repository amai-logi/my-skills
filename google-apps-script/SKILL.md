...
### Input
- Feature requirements from user or from `software-architecture` skill (when it recommends Apps Script over a full-stack app)
- `design-language.md` for styling dialogs and sidebars (read via `frontend` skill patterns)

### Output
- Apps Script code (server-side `.gs`)
- HTML dialogs/sidebars (styled to design language)
- Installation instructions and OAuth consent guidance

### Next skills

| Next | What it receives |
|------|-----------------|
| `frontend` | If the dialog/sidebar needs styling — load `frontend` skill for Apps Script HTML runtime patterns |
| `testing` | Apps Script project to create a manual test checklist and extract testable logic |
...
- **Skill split:** "This skill's [section] is growing complex enough to be its own skill."
- **Skill overlap:** "This skill and `[other skill]` both cover [topic]. Consider consolidating." — e.g., if `frontend` skill's Apps Script HTML section duplicates patterns here.
